...
test.describe('Home page', () => {
  test('should render the welcome heading, intro description, and all call-to-action links', async ({ page }) => {
    ...
    // Assert the intro description paragraph (#description) — the element this change edits.
    // It carries both the base blurb and the content-variant sentence this change introduced.
    const description = page.locator('#description');
    await expect(description).toBeVisible();
    await expect(description).toContainText('A React web application built for Playwright testing.');
    await expect(description).toContainText('Feature branch users/dev130 adds a homepage content variant.');
