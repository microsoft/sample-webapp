...
test.describe('Home page', () => {
  ...
  test('description paragraph renders the expected content variant', async ({ page }) => {
    await page.goto('/');

    const description = page.locator('#description');
    await expect(description).toBeVisible();
    await expect(description).toHaveText(
      'A React web application built for Playwright testing. Feature branch users/dev24 adds a homepage content variant.'
    );
  });
});

// repeat lifecycle prep for PR #130
