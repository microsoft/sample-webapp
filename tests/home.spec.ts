...
test.describe('Home page', () => {
  ...
  test('description paragraph renders the users/dev27 content variant', async ({ page }) => {
    await page.goto('/');

    const description = page.locator('#description');
    await expect(description).toBeVisible();
    await expect(description).toHaveText(
      'A React web application built for Playwright testing. Feature branch users/dev27 adds a homepage content variant.'
    );
  });
});
