...
test.describe('Home page', () => {
  test('should render the welcome heading, description, and all call-to-action links', async ({ page }) => {
    ...
    // The homepage description paragraph carries the content-variant messaging
    // introduced by PR #96. Assert the distinctive new phrase as a substring so
    // unrelated future copy edits don't make this brittle.
    const description = page.locator('#description');
    await expect(description).toBeVisible();
    await expect(description).toContainText(
      'Feature branch users/dev14 adds a homepage content variant'
    );
