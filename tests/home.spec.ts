...
test.describe('Home page', () => {
  ...
  test('description paragraph renders the current homepage messaging', async ({ page }) => {
    await page.goto('/');

    const description = page.locator('#description');
    await expect(description).toBeVisible();
    await expect(description).toHaveText(
      'A React web application built for Playwright testing. Feature branch users/dev93 adds a homepage content variant.'
    );
  });
