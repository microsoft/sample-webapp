...
test.describe('Home page', () => {
  test('should render the welcome heading, intro description, and all call-to-action links', async ({ page }) => {
    ...
    // The #description paragraph is the element this change edits — assert its
    // rendered content-variant copy so a revert/regression fails the suite.
    const description = page.locator('#description');
    await expect(description).toBeVisible();
    await expect(description).toContainText(
      'A React web application built for Playwright testing. Feature branch users/dev136 adds a homepage content variant.'
    );
