...
test.describe('Home page', () => {
  ...
  test('renders the updated description copy', async ({ page }) => {
    await page.goto('/');

    await expect(page.locator('#description')).toHaveText(
      'A React web application built for Playwright testing. Feature branch users/dev76 adds a homepage content variant.'
    );
  });
