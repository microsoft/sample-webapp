...
test.describe('Home page', () => {
  ...
  test('renders the description paragraph with the content-variant copy', async ({ page }) => {
    await page.goto('/');

    const description = page.locator('#description');
    await expect(description).toBeVisible();
    await expect(description).toHaveText(
      'A React web application built for Playwright testing. Feature branch users/dev44 adds a homepage content variant.'
    );
  });
});
