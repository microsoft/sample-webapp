...
test.describe('Home page', () => {
  ...
  test('renders the description paragraph with the homepage content variant', async ({ page }) => {
    await page.goto('/');

    const description = page.getByRole('main').locator('#description');
    await expect(description).toBeVisible();
    await expect(description).toHaveText(
      'A React web application built for Playwright testing. Feature branch users/dev95 adds a homepage content variant.'
    );
  });
});
