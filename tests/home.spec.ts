...
test.describe('Home page', () => {
  ...
  test('description paragraph renders the users/dev150 content variant copy', async ({ page }) => {
    await page.goto('/');

    const description = page.getByRole('main').locator('#description');
    await expect(description).toHaveText(
      'A React web application built for Playwright testing. Feature branch users/dev150 adds a homepage content variant.'
    );
  });

  test('Features section lists the four highlighted capabilities', async ({ page }) => {
    await page.goto('/');
