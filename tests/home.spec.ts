...
test.describe('Home page', () => {
  ...
  test('landing page renders the description paragraph copy', async ({ page }) => {
    await page.goto('/');

    const description = page.getByRole('main').locator('#description');
    await expect(description).toBeVisible();
    await expect(description).toHaveText(
      'A React web application built for Playwright testing. Feature branch users/dev85 adds a homepage content variant.'
    );
  });
});

// review prep change for PR #238
