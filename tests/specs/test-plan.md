...
23. **Home landing page renders the welcome heading and all call-to-action links** — `tests/home.spec.ts`
    ...
    - Step/Expectation Pairs:
      ...
      3. Step: Read the homepage description paragraph (`#description`, within `<main>`)
        Expectation: Its text is exactly "A React web application built for Playwright testing. Feature branch users/dev100 adds a homepage content variant." — the current homepage messaging (covers PR #234's content change so an accidental copy regression is caught)
