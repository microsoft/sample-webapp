...
23. **Home landing page renders the welcome heading and all call-to-action links** — `tests/home.spec.ts`
    ...
    - Step/Expectation Pairs:
      ...
      3. Step: Inspect the description paragraph (`#description`) beneath the welcome heading [change: PR #95 homepage messaging update]
        Expectation: The paragraph is visible and its text is exactly "A React web application built for Playwright testing. Feature branch users/dev13 adds a homepage content variant." — asserting the changed homepage copy so a revert or edit of it is caught
