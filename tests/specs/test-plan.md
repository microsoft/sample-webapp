...
23. **Home landing page renders the welcome heading and all call-to-action links** — `tests/home.spec.ts`
    ...
    - Step/Expectation Pairs:
      ...
      3. Step: Inspect the intro description paragraph (`#description`) _(coverage added for PR #97 — change)_
        Expectation: The `#description` paragraph is visible and its text contains the content-variant copy "Feature branch users/dev15 adds a homepage content variant." (asserts the actual rendered copy the PR added, so an accidental revert of the homepage messaging would fail this test)
