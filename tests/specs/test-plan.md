...
23. **Home landing page renders the welcome heading, intro description, and all call-to-action links** — `tests/home.spec.ts`
    - Attribution: change (the intro-description assertion covers the new homepage content added by PR #299; the heading/CTA assertions are pre-existing regression coverage)
    ...
    - Step/Expectation Pairs:
      ...
      2. Step: Inspect the intro description paragraph (`#description`)
        Expectation: It is visible and its text contains the homepage content variant copy — "Feature branch users/dev138 adds a homepage content variant" — confirming PR #299's messaging update renders on the landing page
