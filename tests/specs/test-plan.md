...
37. **Feedback comment character counter reflects length and enforces the 300-character limit** — _removed (heal): the `/feedback` page has no comment character counter or 300-char `maxLength` cap — the feature does not exist in `Feedback.js` (rating + optional comment only), so the test asserted phantom behavior._
38. **Feedback Clear button resets rating, comment, counter, and the success message** — _removed (heal): the `/feedback` page has no "Clear" button — `Feedback.js` renders only the rating radios, comment textarea, and "Send feedback" submit, so the test asserted phantom behavior._
