...
_Removed (heal, 2026-08-06): entries 37 "Feedback comment character counter…" and 38 "Feedback Clear button resets…" — these asserted a character counter, 300-char input cap, and a Clear button that the Feedback page (`src/pages/Feedback.js`) has never implemented and, per its documented scope (rating + optional comment with validation), was never intended to. They were introduced by a faulty scout run (PR #339) that claimed the features existed; the tests could never pass against the real component, so they were retired as stale._
