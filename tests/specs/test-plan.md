...
37. **Feedback comment character counter reflects length and enforces the 300-character limit** — `tests/feedback.spec.ts` (change)
    - REMOVED (heal): the Feedback page has no comment character counter and no 300-char `maxLength` cap — the feature never existed in `Feedback.js` in any commit (its scope is a rating plus an optional comment with validation). This test was added by a scout run (#339) asserting phantom behavior, not catching a real regression, so it was deleted.
38. **Feedback Clear button resets rating, comment, counter, and the success message** — `tests/feedback.spec.ts` (change)
    - REMOVED (heal): the Feedback page has no "Clear" reset button — it exposes only the rating radios, the comment textarea, and a "Send feedback" submit button. The button never existed in any commit; this test (also from scout run #339) asserted phantom behavior, so it was deleted.
