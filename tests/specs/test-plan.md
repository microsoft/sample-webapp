...
37. **Home landing page renders the description paragraph copy** — `tests/home.spec.ts` _(covers PR #221 change)_
    ...
    - Step/Expectation Pairs:
      ...
      2. Step: Read the description paragraph's text
        Expectation: It reads the full intended copy — the base "A React web application built for Playwright testing." sentence followed by the content-variant sentence "Feature branch users/dev87 adds a homepage content variant." (asserts the actual rendered copy, so a revert or edit of the message would fail)
