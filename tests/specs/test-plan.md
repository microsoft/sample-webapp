...
37. **Feedback comment character counter reflects length and enforces the 300-character limit** — `tests/feedback.spec.ts` (removed)
    - Removed by heal (PR #352 run): asserted a live comment character counter and a 300-character `maxLength` cap that `Feedback.js` never implemented (the form is only a 1–5 rating radiogroup plus an optional, uncapped comment). Phantom feature from a prior scout run — not a real regression — so the stale test and this plan entry were removed.
38. **Feedback Clear button resets rating, comment, counter, and the success message** — `tests/feedback.spec.ts` (removed)
    - Removed by heal (PR #352 run): asserted a "Clear" reset button that `Feedback.js` never had (the form exposes only "Send feedback"). Phantom feature from a prior scout run — not a real regression — so the stale test and this plan entry were removed.
