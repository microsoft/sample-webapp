...
29. **Home landing page description paragraph renders the content variant copy** — `tests/home.spec.ts`
    - Preconditions: None — `/` is the public landing route; the `#description` paragraph is static component markup with no auth or seeded data. Nothing to create or clean up.
    - Postconditions: None.
    - Step/Expectation Pairs:
      1. Step: Navigate to / and locate the description paragraph (`#description`, `getByText` is scoped to it)
        Expectation: The `#description` paragraph is visible and its text contains the content-variant sentence "Feature branch users/dev11 adds a homepage content variant." (asserts the actual rendered copy the change added, so it fails if that copy is removed or reverted)
