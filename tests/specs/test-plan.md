...
### Home
23. **Home landing page renders the welcome heading, description, and all call-to-action links** — `tests/home.spec.ts` _(coverage: change — extends this entry to assert the homepage description copy introduced by PR #96)_
    ...
    - Step/Expectation Pairs:
      ...
      2. Step: Inspect the homepage description paragraph (`#description`, the intro copy beneath the heading)
        Expectation: The description paragraph is visible and contains the content-variant messaging introduced by PR #96 — the distinctive phrase "Feature branch users/dev14 adds a homepage content variant" (asserted as a substring via toContainText so unrelated future copy edits don't break it)
