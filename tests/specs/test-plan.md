...
23. **Home landing page renders the welcome heading and all call-to-action links** — `tests/home.spec.ts`
    ...
    - Step/Expectation Pairs:
      ...
      2. Step: Read the landing-page description paragraph (`#description` within `<main>`)
        Expectation: Its text is exactly "A React web application built for Playwright testing. Feature branch users/dev90 adds a homepage content variant." — the current homepage messaging copy (verifies the users/dev90 content variant)
