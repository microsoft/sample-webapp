...
37. **Feedback comment character counter reflects length and enforces the 300-character limit** — `tests/feedback.spec.ts` (removed)
    - Removed by heal: the asserted 300-character counter and `maxLength` cap have never existed in `src/pages/Feedback.js` in any commit (the component is only a rating radiogroup plus an optional, uncapped comment textarea). This entry and its test were added by a prior scout run for a feature that was never implemented; the test asserted a phantom feature, not a real regression, so the test was removed.
38. **Feedback Clear button resets rating, comment, counter, and the success message** — `tests/feedback.spec.ts` (removed)
    - Removed by heal: the asserted "Clear" button has never existed in `src/pages/Feedback.js` (the form has only a "Send feedback" submit button). Like entry 37, this was a prior-scout phantom-feature test asserting behavior the app never had; the test was removed rather than realigned.
