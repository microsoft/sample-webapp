...
### Home
23. **Home landing page renders the welcome heading, description, and all call-to-action links** — `tests/home.spec.ts`
    - Coverage attribution: Step 1's description assertion covers the change (PR #228 updated the `#description` copy); the heading and CTA assertions are existing regression coverage.
    ...
    - Step/Expectation Pairs:
      1. Step: Navigate to /
        ...
        Expectation: The description paragraph (`#description`) is visible and its text contains "Feature branch users/dev94 adds a homepage content variant." — asserting the actual rendered copy the change introduced (would fail if the content reverted or regressed)
