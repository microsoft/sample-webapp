...
23. **Home landing page renders the welcome heading, intro description, and all call-to-action links** — `tests/home.spec.ts`
    ...
    - Step/Expectation Pairs:
      ...
      3. Step: Read the intro description paragraph (`#description`) beneath the heading [coverage: change — PR #231 homepage content variant]
        Expectation: The `#description` paragraph is visible and its text contains the homepage content variant copy "Feature branch users/dev97 adds a homepage content variant." (asserts the exact user-visible content the change added; would fail if the change is reverted)
