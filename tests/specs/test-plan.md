...
34. **Home description paragraph renders the content-variant copy** — `tests/home.spec.ts`  _(change)_
    ...
    - Step/Expectation Pairs:
      1. Step: Navigate to /
        Expectation: The `#description` paragraph is visible
      2. Step: Read the `#description` paragraph text
        Expectation: It renders the exact content-variant copy the change introduces — "A React web application built for Playwright testing. Feature branch users/dev29 adds a homepage content variant." — so a regression or revert of the homepage messaging would fail the test
