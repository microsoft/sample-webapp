...
37. **Home landing page renders the description paragraph text** — `tests/home.spec.ts` <!-- attribution: change -->
    ...
    - Step/Expectation Pairs:
      1. Step: Navigate to /
        Expectation: The description paragraph (`#description`, within `<main>`) is visible with the exact text "A React web application built for Playwright testing. Feature branch users/dev146 adds a homepage content variant."
