...
23. **Home landing page renders the welcome heading, intro description, and all call-to-action links** — `tests/home.spec.ts`
    ...
    - Step/Expectation Pairs:
      ...
      2. Step: Inspect the intro description paragraph (`#description`) in the main content
        Expectation: The paragraph is visible and its text contains both the base blurb "A React web application built for Playwright testing." and the content-variant sentence "Feature branch users/dev135 adds a homepage content variant." (the copy this change introduces; a revert or regression would fail here)
