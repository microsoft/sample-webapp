...
23. **Home landing page renders the welcome heading and all call-to-action links** — `tests/home.spec.ts`
    ...
    - Step/Expectation Pairs:
      ...
      3. Step: Inspect the homepage description paragraph (`#description`)
        Expectation: The `#description` paragraph is visible and reads "A React web application built for Playwright testing. Feature branch users/dev1 adds a homepage content variant." (asserts the copy this PR introduced; `[coverage: change]`)
